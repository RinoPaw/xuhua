...
- 轻量检索：根据用户输入实时召回相关条目，支持分类限定与详情查看。
...
项目不依赖 PySide6、FAISS、PyAudio、Vosk、Edge-TTS 等旧桌面端组件。浏览器语音播报使用系统和浏览器自带的 Web Speech API，实际音色取决于本机环境。
...
| `/api/items` | GET | 条目检索，支持 `q`、`category`、`limit`、`offset` |
